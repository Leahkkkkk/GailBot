...
@dataclass
class ANALYSIS_MARKER(DataclassFromDict): 
...
def load_label():
    d = toml.load(os.path.join(os.path.dirname(__file__), "config.toml"))
...
def load_marker():
    d = toml.load(os.path.join(os.path.dirname(__file__), "config.toml"))
    return  ANALYSIS_MARKER.from_dict(d["ANALYSIS_MARKER"])
