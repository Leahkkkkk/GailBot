...
class TranscriptionStage:
    ...
    def generate_utterances(self) -> None:
        ...
        for _ , payload in payloads.items():
            self.thread_pool.add_task(
                self._extract_source_audios, [payload], {})
        self.thread_pool.wait_completion()
        # Transcribe all sources next.
        for _ , payload in payloads.items():
            self.thread_pool.add_task(
                self._transcribe_thread,[payload],{})
        self.thread_pool.wait_completion()
    ...
    def _extract_source_audios(self, payload : SourcePayload) -> None:
        ...
        payload.source_to_audio_map = source_to_audio_map
    ...
    def _transcribe_thread(self, payload : SourcePayload) -> None:
        ...
        for source_file_name, _ in payload.source_to_audio_map.items():
        ...
        if all(list(source_status_map.values())):
            ...
            payload.conversation.set_utterances(utterances_map)
            # The stage has been successful for this source.
            payload.transcription_successful = True
        else:
            payload.conversation.set_transcription_status(
                TranscriptionStatus.unsuccessful)
    ...
    def _transcribe_watson_thread(self, source_file_name : str,
        ...
        source_path = payload.source_to_audio_map[source_file_name]
        settings : GailBotSettings = payload.conversation.get_settings()
        engine.configure(
    ...
    def _can_transcribe_source(self, payload : SourcePayload) -> bool:
        for _, audio_path in payload.source_to_audio_map.items():
            if audio_path == None:
                return False
        return True