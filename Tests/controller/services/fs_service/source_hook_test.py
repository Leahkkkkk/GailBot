...
def test_write_to_file() -> None:
    ...
    io = IO()
    hook = SourceHook(SOURCE_DIR_PATH, SOURCE_NAME,EMPTY_DIR_PATH)
    assert hook.write_to_file("test","permanent","apple","txt","Test dadasd")
    assert hook.write_to_file("test","permanent","apple","txt","This is a new line")
    assert hook.write_to_file("test","permanent","apple","txt","Test dadasd",True)
