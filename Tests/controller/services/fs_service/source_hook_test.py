...
def test_add_to_source() -> None:
    ...
    """
    io = IO()
    hook = SourceHook(SOURCE_DIR_PATH, SOURCE_NAME)
    assert hook.add_to_source("wav",WAV_FILE_PATH,True)
    assert io.is_file(hook.get_item_path("wav"))
    assert not hook.add_to_source("wav",WAV_FILE_PATH,False)
...
def test_cleanup() -> None:
    ...
    assert hook.add_to_source("dir",EMPTY_DIR_PATH,False)
    hook.cleanup()
    assert io.number_of_files_in_directory(hook.get_path(),["*"],False)[1] == 0
...
def test_get_path() -> None:
    ...
    io = IO()
    hook = SourceHook(SOURCE_DIR_PATH, SOURCE_NAME)
    assert io.is_directory(hook.get_path())
